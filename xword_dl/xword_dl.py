...
def by_keyword(keyword: str, **kwargs) -> tuple[Puzzle, str]:
    selected_downloader = next(
        (d for d in get_supported_outlets(command_only=True) if d.command == keyword), None
    )
...
def get_supported_outlets(command_only=False, matches_url=False, matches_embed_url=False):
    matched_plugins = []
    for plugin in plugins:
        if command_only and not plugin.command:
            continue
        if matches_url and getattr(plugin.matches_url, "__func__") is getattr(__bd.matches_url, "__func__"):
            continue
        if matches_embed_url and getattr(plugin.matches_embed_url, "__func__") is getattr(__bd.matches_embed_url, "__func__"):
            continue
...
def get_help_text_formatted_list():
    text = ""
    for d in sorted(get_supported_outlets(command_only=True), key=lambda x: x.outlet.lower()):
...
def main():
    ...
    if args.authenticate and args.source:
        selected_downloader = next(
            (d for d in get_supported_outlets(command_only=True) if d.command == args.source), None
        )
